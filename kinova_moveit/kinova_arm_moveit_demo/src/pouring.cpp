...
void PickPlace::define_joint_values()
{
    ...
    start_joint_.resize(joint_names_.size());
    //    getInvK(start_pose_, start_joint_);
    start_joint_[0] = 4.102531216027051;
    start_joint_[1] = 3.8992787616322038;
    start_joint_[2] = 5.730599621012429;
    start_joint_[3] = 0.806006369702754;
    start_joint_[4] = 2.5400215812800453;
    start_joint_[5] = 4.656019980119854;
    start_joint_[6] = 6.599038673552432;

    grasp_joint_.resize(joint_names_.size());
    //    getInvK(grasp_pose, grasp_joint_);
    grasp_joint_[0] = 4.859162179645573;
    grasp_joint_[1] = 3.9551393644449426;
    grasp_joint_[2] = 6.255259932621233;
    grasp_joint_[3] = 1.2862667259338663;
    grasp_joint_[4] = 3.822310476651715;
    grasp_joint_[5] = 4.149510176604376;
    grasp_joint_[6] = 5.731502432621967;

    pregrasp_joint_.resize(joint_names_.size());
    //    getInvK(pregrasp_pose, pregrasp_joint_);
    pregrasp_joint_[0] = 4.161106911571126;
    pregrasp_joint_[1] = 3.9529627628860213;
    pregrasp_joint_[2] = 5.819103388254597;
    pregrasp_joint_[3] = 1.0883457203042757;
    pregrasp_joint_[4] = 2.656279481822473;
    pregrasp_joint_[5] = 4.4027118151403135;
    pregrasp_joint_[6] = 6.598125333286831;

//    postgrasp_joint_ = pregrasp_joint_;
    postgrasp_joint_.resize(joint_names_.size());
    //    getInvK(pregrasp_pose, postgrasp_joint_);
    postgrasp_joint_[0] = 4.858621557944287;
    postgrasp_joint_[1] = 3.955146288663777;
    postgrasp_joint_[2] = 6.2623231684643414;
    postgrasp_joint_[3] = 1.2862957543897482;
    postgrasp_joint_[4] = 3.824841811267835;
    postgrasp_joint_[5] = 4.155194960267259;
    postgrasp_joint_[6] = 3.754880036799235;
...
{
    ROS_INFO_STREAM("Bringing it to home for the first time");
    clear_workscene();
    ros::WallDuration(1.0).sleep();
    build_workscene();
    ros::WallDuration(1.0).sleep();
    ...
    while(ros::ok()) {
    ROS_INFO_STREAM("Press any key to execute task");
    std::cin >> pause_;
    ...
    ROS_INFO_STREAM("Planning to go to home position ...");
    clear_workscene();
    ros::WallDuration(1.0).sleep();
    build_workscene();
    ros::WallDuration(1.0).sleep();
